...
public class BlockVespeneCondenser extends BlockContainer implements IPylonPowered
{
	...
	public BlockVespeneCondenser(boolean isActive)
	{
		super(Material.rock);
	...
	{
		this.blockIcon = icon.registerIcon(ModInfo.MODID + ":" + "vespenecondensericon");
	}

	@Override
	public Item getItemDropped(int par1, Random random, int par3)
	{
		return Item.getItemFromBlock(PokemonMDBlock.vespeneCondenser);
	}
	...
	{	
		if(!world.isRemote)
		{
			player.openGui(PokemonMDMain.modInstance, PokemonMDGuiHandler.guiIDVespeneCondenser, world, x, y, z);
		}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityPlayer player, ItemStack itemStack)
	{
		...
		{
			((TileEntityVespeneCondenser)world.getTileEntity(x, y, z)).setGuiDisplayName(itemStack.getDisplayName());
		}
	}

	public static void updateVespeneCondenserBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord)
	{
		...
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, PokemonMDBlock.vespeneCondenserActive);
		}
		else
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, PokemonMDBlock.vespeneCondenser);
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) 
	{
		if (!keepInventory) 
		{
			TileEntityVespeneCondenser tileEntity = (TileEntityVespeneCondenser) world.getTileEntity(x, y, z);
