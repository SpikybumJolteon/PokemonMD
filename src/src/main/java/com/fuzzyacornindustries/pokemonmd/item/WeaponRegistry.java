...
/**
 * 
 * Referenced from Zelda Sword Skills by CoolAlias.
 * 
...
public class WeaponRegistry
{
	...
	public static final String IMC_SWORD_KEY = "PokemonMDRegisterSword";

	/** FML Inter-Mod Communication key for registering items as non-sword weapons */
	public static final String IMC_WEAPON_KEY = "PokemonMDRegisterWeapon";
	...
	private void processArray(String[] names, String origin, boolean isSword, boolean register) 
	{
		for (String s : names) 
		{
			...
			if (parts != null) 
			{
				...
				if (item == null) 
				{
					PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] %s:%s could not be found - the mod may not be installed, or you may have typed it incorrectly", origin, parts[0], parts[1]));
				} 
				else if (isSword) 
				{
					if (register) 
	...
	public boolean registerSword(String origin, String modid, Item item) 
	{
		...
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] CONFLICT: %s:%s cannot be registered as a sword - it is already registered as a non-sword weapon", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		} 
		else if (swords.add(item)) 
		{
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] Registered %s:%s as a sword", origin, modid, item.getUnlocalizedName().replace("item.", "")));
			added = true;
		} 
		else 
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] %s:%s has already been registered as a sword", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
	...
	public boolean registerWeapon(String origin, String modid, Item item) 
	{
		...
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] CONFLICT: %s:%s cannot be registered as a weapon - it is already registered as a sword", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		} 
		else if (weapons.add(item)) 
		{
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] Registered %s:%s as a non-sword weapon", origin, modid, item.getUnlocalizedName().replace("item.", "")));
			added = true;
		} 
		else 
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] %s:%s has already been registered as a weapon", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
	...
	public boolean removeSword(String origin, String modid, Item item) 
	{
		...
		{
			added = true;
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] %s:%s added to FORBIDDEN swords list", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
		
		if (swords.remove(item))
		{
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] Removed %s:%s from list of registered SWORDS", origin, modid, item.getUnlocalizedName().replace("item.", "")));
			return true;
		} 
		else if (!added) 
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] Could not remove %s:%s - it was not registered as a sword", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
		return added;
	}
	...
	public boolean removeWeapon(String origin, String modid, Item item) 
	{
		...
		{
			added = true;
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] %s:%s added to FORBIDDEN weapons list", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
		
		if (weapons.remove(item)) 
		{
			PokemonMDMain.logger.info(String.format("[WeaponRegistry] [%s] Removed %s:%s from list of registered WEAPONS", origin, modid, item.getUnlocalizedName().replace("item.", "")));
			return true;
		} 
		else if (!added) 
		{
			PokemonMDMain.logger.warn(String.format("[WeaponRegistry] [%s] Could not remove %s:%s - it was not registered as a non-sword weapon", origin, modid, item.getUnlocalizedName().replace("item.", "")));
		}
		return added;
	}
	...
	public static String[] parseString(String itemid) 
	{
		...
		PokemonMDMain.logger.error(String.format("[WeaponRegistry] String must be in the format 'modid:registered_item_name', received: %s", itemid));
