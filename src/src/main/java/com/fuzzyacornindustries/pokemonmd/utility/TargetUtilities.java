...
public class TargetUtilities
{
	...
	public static final List<EntityMob> acquireAllLookMobTargets(EntityLivingBase seeker, int distance, double radius) 
	{
		...
		while ((int) distanceTraveled < distance) 
		{
			...
			List<EntityMob> list = seeker.worldObj.getEntitiesWithinAABB(EntityMob.class,
					AxisAlignedBB.getBoundingBox(targetX-radius, targetY-radius, targetZ-radius, targetX+radius, targetY+radius, targetZ+radius));
	...
	public static final List<EntityMob> acquireNearestLookMobTargetList(EntityLivingBase seeker, int distance, double radius, int numberOfTargets) 
	{
		...
		while(counter < numberOfTargets && !isListExhausted)
		{
			...
			for (EntityMob target : targets) 
			{
				...
				{
					double newDistance = target.getDistanceSqToEntity(seeker);
	...
	public static final void applyHunterBoltSettings(EntityHunterBolt bolt, ItemStack bow, float charge, float modifier) 
	{
		...
		int k = Math.round(EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, bow) * modifier);

		if (k > 0) { bolt.setDamage(bolt.getDamage() + (double) k * 0.5D + 0.5D); }

		int l = Math.round(EnchantmentHelper.getEnchantmentLevel(Enchantment.punch.effectId, bow) * modifier);

		if (l > 0) { bolt.setKnockbackStrength(l); }

		if (EnchantmentHelper.getEnchantmentLevel(Enchantment.flame.effectId, bow) > 0 && modifier >= 1F) 
		{
			bolt.setFire(100);
		}
	}
