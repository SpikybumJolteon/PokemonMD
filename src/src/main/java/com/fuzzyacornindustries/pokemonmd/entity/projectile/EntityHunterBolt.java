 ...
 */
public class EntityHunterBolt extends EntityArrow implements IProjectile, IEntityAdditionalSpawnData
{
	...
	public EntityHunterBolt(World world, EntityLivingBase shooterEntity, EntityLivingBase targetEntity, float baseSpeed, float accuracy)
	{
		...
		this.posY = shooterEntity.posY + (double)shooterEntity.getEyeHeight() - 0.10000000149011612D;
		double d0 = targetEntity.posX - shooterEntity.posX;
		double d1 = (targetEntity.boundingBox.minY + (double)(targetEntity.height / 3.0F) - this.posY) * 1.1D;
		...
		if (d3 >= 1.0E-7D)
		{
			...
			this.setLocationAndAngles(shooterEntity.posX + d4, this.posY, shooterEntity.posZ + d5, f2, f3);
			this.yOffset = 0.0F;
			float f4 = (float)d3 * 0.1F;

			this.setThrowableHeading(d0, d1 + (double)f4, d2, baseSpeed, accuracy);
		}
	...
	@SideOnly(Side.CLIENT)
	@Override
	public void setPositionAndRotation2(double xPos, double yPos, double zPos, float yawPar, float pitchPar, int p_70056_9_)
	{
	...
	public void setVelocity(double xVelocity, double yVelocity, double zVelocity)
	{
		...
		{
			float horizontalMagnitude = MathHelper.sqrt_double(xVelocity * xVelocity + zVelocity * zVelocity);
	...
	{
		++ticksInAir;
		MovingObjectPosition mop = TargetUtilities.checkForImpact(worldObj, this, getShooter(), 0.3D, ticksInAir >= 5);
	...
	protected void onImpactBlock(MovingObjectPosition mop) 
	{
		...
		this.playSound(ModInfo.MODID + ":effects.bowofkindred.hit", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
	...
	{
		if (mop.entityHit != null && !(mop.entityHit instanceof TamablePokemon) && mop.entityHit != this.shootingEntity) 
		{
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound)
	{
		...
		shooterName = nbttagcompound.getString("shooter");
		...
		//dataWatcher.updateObject(TARGET_DATAWATCHER_INDEX, nbttagcompound.hasKey("target") ? nbttagcompound.getInteger("target") : -1);
	}
	 ...
	 */
	public void setIsCritical(boolean criticalHitBoolean)
	{
		byte b0 = this.dataWatcher.getWatchableObjectByte(16);

		if (criticalHitBoolean)
		{
