...
import com.fuzzyacornindustries.pokemonmd.model.mob.ModelOkamiEspeon;
import com.fuzzyacornindustries.pokemonmd.model.mob.tools.ModelOkamiSunGlaive;
...
public class RenderOkamiEspeon extends RenderLiving
{
	private static final ResourceLocation mainTexture = new ResourceLocation(ModInfo.MODID + ":" +"textures/mobs/tamable/okamiespeon.png");
	private static final ResourceLocation hurtTexture = new ResourceLocation(ModInfo.MODID + ":" +"textures/mobs/tamable/okamiespeonhurt.png");
	private static final ResourceLocation happyTexture = new ResourceLocation(ModInfo.MODID + ":" +"textures/mobs/tamable/okamiespeonhappy.png");

	private static final ResourceLocation sunGlaiveTexture = new ResourceLocation(ModInfo.MODID + ":" +"textures/mobs/tamable/okamiweapon/sunglaive.png");

	static float castShadowSize = 0.4F;

	private ModelOkamiSunGlaive modelSunGlaive;

	public RenderOkamiEspeon()
	{	
		super(new ModelOkamiEspeon(), castShadowSize);

		this.modelSunGlaive = new ModelOkamiSunGlaive();
	}

	protected ResourceLocation getEntityTexture(EntityOkamiEspeon par1Entity)
	{
	...
	public void doRender(EntityLiving entity, double dx, double dy, double dz, float yaw, float partialTick)
	{
		...
		this.modelSunGlaive.setLivingAnimations(entity, distanceMoved, horzVelocity, partialTick);
		this.renderSunGlaive(entity, distanceMoved, horzVelocity, yawRotationDifference, yawHead - yawOffset, pitchRotation, modelSize);
		int colorMultiplier;
		float redColor;
		float greenColor;
		float alphaFloat;
		...
		if ((colorMultiplier >> 24 & 255) > 0 || entity.hurtTime > 0 || entity.deathTime > 0)
		{
			...
			if (entity.hurtTime > 0 || entity.deathTime > 0)
			{
				...
				this.modelSunGlaive.render(entity, distanceMoved, horzVelocity, yawRotationDifference, yawHead - yawOffset, pitchRotation, modelSize);
		...
		/*
		System.out.println( "Test Animation Tick Code Sun Glaive doRender method" );
	...
	protected void renderSunGlaive(EntityLivingBase entity, float distanceMoved, float horzVelocity, float yawRotationDifference, float yaw, float pitchRotation, float modelSize)
	{
		GL11.glPushMatrix();
		GL11.glEnable(GL11.GL_BLEND);
		...
		this.modelSunGlaive.render(entity, distanceMoved, horzVelocity, yawRotationDifference, yaw, pitchRotation, modelSize);
		GL11.glDisable(GL12.GL_RESCALE_NORMAL);
		...
		{
			this.modelSunGlaive.setRotationAngles(distanceMoved, horzVelocity, yawRotationDifference, yaw, pitchRotation, modelSize, entity);
		}*/
	}
	...
	{
		return this.getEntityTexture((EntityOkamiEspeon)par1Entity);
	}
}