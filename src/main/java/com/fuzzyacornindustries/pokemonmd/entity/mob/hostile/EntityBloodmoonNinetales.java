...
import com.fuzzyacornindustries.pokemonmd.entity.mob.ai.AIBloodmoonNinetalesFireball;
import com.fuzzyacornindustries.pokemonmd.entity.mob.ai.AIBloodmoonNinetalesFireblast;
import com.fuzzyacornindustries.pokemonmd.entity.mob.ai.AIBloodmoonNinetalesFoxfireStorm;
import com.fuzzyacornindustries.pokemonmd.entity.mob.ai.AIBloodmoonNinetalesFoxfireSummon;
import com.fuzzyacornindustries.pokemonmd.entity.mob.ai.AIBloodmoonNinetalesJumpFireball;
...
public class EntityBloodmoonNinetales extends EntityHostilePokemonMD implements IRangedAttackMob, IAnimateAhriNinetales, IMinorBoss, IStunImmune, IMobMotionTracker
{
	...
	static String mobName = "bloodmoonninetales";
	...
	protected String getLivingSound()
	{
		...
		{
			return ModInfo.MODID + ":mob." + getMobName() + ".laugh";
		}
		else
		{
			return null;
		...
		{	
			...
			else
			{
				...
				else if(attackWeight < 2 && !this.isWet() && !targetEntity.isWet())
				{
					...
					if(!isInLiquid())
					{
						PacketDispatcher.sendAnimationPacket(this, LibraryAhriNinetalesAttackID.FOXFIRE_SUMMON);
					}
					else
					{
				...
				else if(attackWeight < 6 && (double)this.getHealth() <= this.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue() / 2D)
				{
					...
					PacketDispatcher.sendAnimationPacket(this, LibraryAhriNinetalesAttackID.FOXFIRE_STORM);
				}
				else if(attackWeight < 13 && !isInLiquid())
				{
					...
					if (block.isAir(worldObj, j2, k, l))
					{
						...
						{
							PacketDispatcher.sendAnimationPacket(this, LibraryAhriNinetalesAttackID.FIREBLAST);	
						}
						else
						{
	...
	{
		this.worldObj.playSoundAtEntity(this, ModInfo.MODID + ":mob." + getMobName() + ".attack", 1.0F, 1.0F + this.worldObj.rand.nextFloat() * 0.2F);
	}

	public void playExertSelfSound()
	{
		this.worldObj.playSoundAtEntity(this, ModInfo.MODID + ":mob." + getMobName() + ".exertself", 1.0F, 1.0F + this.worldObj.rand.nextFloat() * 0.2F);
	}

	public void playGaspSound()
	{
		this.worldObj.playSoundAtEntity(this, ModInfo.MODID + ":mob." + getMobName() + ".gasp", 1.0F, 1.0F + this.worldObj.rand.nextFloat() * 0.2F);
	}

	public void playLaughSound()
	{
		this.worldObj.playSoundAtEntity(this, ModInfo.MODID + ":mob." + getMobName() + ".laugh", 1.0F, 1.0F + this.worldObj.rand.nextFloat() * 0.2F);
	}
